...
from Scripts import transfer_learning
#
...
FILTERS_DICT = {
    ...
    "erode": ImageFilter.MinFilter,
    "dilate": ImageFilter.MaxFilter,
}
...
def apply_filters(image, zone, filter, mode):
    if filter == "cut":
        if mode == "select":
            ...
            image= Image.composite(image,shadow, mask=zone)
    

    
    elif filter == "model":
        image = segmentation.multiclass_segmentation(image,"deep")
    
    elif filter == "conv":
        image = functions.CNN(image,"Conv")
    
    elif filter == "MaxPool":
        image = functions.CNN(image,"MaxPool")
    
    elif filter == "AvgPool":
        image = functions.CNN(image,"AvgPool")
    
    elif filter == "All":
        image = functions.CNN(image,"All")
    
    elif filter == "cat":
        clase = transfer_learning.predict(image)
        fontsize=100
        font = ImageFont.truetype("arial.ttf", fontsize)
        if clase == "cat":
            
            d = ImageDraw.Draw(image)
            d.text((10,10), "Gato", fill=(255,255,0),font=font)
        else:
            
            d = ImageDraw.Draw(image)
            d.text((10,10), "Perro", fill=(255,255,0),font=font)
...
def apply_enhancements(image, zone, enhancement, enhancement_factor, mode):
    if enhancement == "scale":
        image = image.resize( int(enhancement_factor * s) for s in image.size ) # Si la operación es escalar
    
    elif enhancement == "rotate":
        image = image.rotate(enhancement_factor)
    
    elif enhancement == "quantization":
        image = image.quantize(colors=enhancement_factor, method=2).convert('RGB')
    
    elif enhancement == "kmeans":
        image = segmentation.multiclass_segmentation(image,"kmeans",classes_number =enhancement_factor)
